...
if [ "$DEBUG_MODE" = "1" ]; then
	if [ -d $DEBUG_CRASHLOG_FOLDER ]; then
		cecho "$DEBUG_CRASHLOG_FOLDER exist - Starting" $green
	else 
	...
	while true; do
	echo "run -c $LAUNCHER_WORLDSERVER_CONF" > $DEBUG_CRASHLOG_FOLDER/core.gdb
	echo "bt" >> $DEBUG_CRASHLOG_FOLDER/core.gdb
	echo "bt full" >> $DEBUG_CRASHLOG_FOLDER/core.gdb
	echo "info thread" >> $DEBUG_CRASHLOG_FOLDER/core.gdb
	echo "thread apply all backtrace full" >> $DEBUG_CRASHLOG_FOLDER/core.gdb
	until gdb ./$LAUNCHER_WORLDSERVER_BIN -x $DEBUG_CRASHLOG_FOLDER/core.gdb --batch >$DEBUG_CRASHLOG_FOLDER/core.log 2>$DEBUG_CRASHLOG_FOLDER/core.err
	grep -B 10 -A 1800 "SIG" "$DEBUG_CRASHLOG_FOLDER/core.log" >> "$DEBUG_CRASHLOG_FOLDER/coretrace_$(date +%Y%m%d-%H:%M).log"
	#cat "coretrace.log" | ./paster | grep "http" >> "core.link"
	cat "$DEBUG_CRASHLOG_FOLDER/core.err" >> "$DEBUG_CRASHLOG_FOLDER/coreerror.log"
	echo "Crash... restarting!" >> $DEBUG_CRASHLOG_FOLDER/core.log
	do sleep 31
	done
	done
	echo "end"
