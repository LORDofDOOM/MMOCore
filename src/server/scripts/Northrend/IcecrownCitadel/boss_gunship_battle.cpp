...
//Functions to define the Combatbox
bool isOutOfGunshipOrgrimmarCombatbox(bool defender, float x,float y)
{
    if (defender)
    {    
        if (x < -391.6629f && x > -494.6246f && y < 2427.1572f && y > 2367.1479f)
            return false;
    }
    else
    {
        if (x < -336.5653f && x > -449.8938f && y < 2020.3637f && y > 1962.7512f)
            return false;
    }
    return true;
}

bool isOutOfGunshipStormwindCombatbox(bool defender, float x,float y)
{
    if (defender)
    {
        if (x < -333.3013f && x > -449.4908f && y < 2089.5036f && y > 2046.2763f)
            return false;
    }
    else
    {
        if (x < -393.3702f && x > -507.812f && y < 2498.5578f && y > 2453.6586f)
            return false;
    }
    return true;
}
...
void RemoveInfightBug(Transport* t1)
{
    Map* map = t1->GetMap();
    for (Map::PlayerList::const_iterator itr = map->GetPlayers().begin(); itr != map->GetPlayers().end(); ++itr)
    {
...
class npc_muradin_gunship : public CreatureScript
{
    public:
        ...
        struct npc_muradin_gunshipAI : public ScriptedAI
        {
            ...
            {
                m_team = _instance->GetData(DATA_TEAM_IN_INSTANCE);
            }
            ...
            void UpdateAI(const uint32 diff)
            {
                ...
                if (!HealthAbovePct(75) && m_team == HORDE)
                {
                            ...
                            if (UpdateVictim())
                                if (me->getVictim())
                                    if ((me->getVictim()->IsWithinDistInMap(me, 50.0f, false)) &&
                                         !me->getVictim()->IsWithinDistInMap(me, 2.0f, false)) // Todo: Fix the distance
                                    {
                ...
                //Check if we are on our ship
                if (CombatBoxCheck)
                {
                    if (me->getVictim())
                        if (m_team == HORDE)
                        {
                            if(isOutOfGunshipStormwindCombatbox(false, me->GetPositionX(), me->GetPositionY()))
                                EnterEvadeMode();
                        }
                        else if (m_team == ALLIANCE)
                        {
                            if(isOutOfGunshipStormwindCombatbox(true, me->GetPositionX(), me->GetPositionY()))
                                EnterEvadeMode();
                        }
                    CombatBoxCheck = 2000;
                }else CombatBoxCheck -= diff;
    ...
    public:
        ...
        struct npc_sergeantAI : public ScriptedAI
        {
            ...
            void Reset()
            {
                ...
                events.ScheduleEvent(EVENT_COMBAT_BOX,3000);
                m_team = _instance->GetData(DATA_TEAM_IN_INSTANCE);
            }
            ...
            bool CanAIAttack(Unit const* target) const
            {
                if (target->GetEntry() == NPC_GB_GUNSHIP_HULL 
                    ||target->GetEntry() == NPC_GB_ALLIANCE_CANON
                    ||target->GetEntry() == NPC_GB_HORDE_CANON)
                    return false;
            ...
            void UpdateAI(const uint32 diff)
            {
                ...
                while (uint32 eventId = events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        case EVENT_COMBAT_BOX:
                            if (m_team == HORDE)
                            {
                                if(isOutOfGunshipOrgrimmarCombatbox(true, me->GetPositionX(), me->GetPositionY()))
                                {
                                    EnterEvadeMode();
                                    events.ScheduleEvent(EVENT_WALK_MOBS, 1500);
                                }
                        ...
                        case EVENT_WALK_MOBS:
                            ...
                            switch (me->GetEntry())
                            {
                                case NPC_GB_KORKRON_SERGANTE:
                                    ...
                                    {
                                        if (Unit* target = me->GetUnit(*me, _instance->GetData64(DATA_GB_MURADIN_BRONZEBEARD)))
                                        {
                                            me->GetMotionMaster()->MoveChase(target);
                                            me->AI()->AttackStart(target);
                                        }
                                ...
                                case NPC_GB_SKYBREAKER_SERGANTE:
                                    ...
                                    {
                                        if (Unit* target = me->GetUnit(*me, _instance->GetData64(DATA_GB_HIGH_OVERLORD_SAURFANG)))
                                        {
                                            me->GetMotionMaster()->MoveChase(target);
                                            me->AI()->AttackStart(target);
                                        }
...
class npc_saurfang_gunship : public CreatureScript
{
    public:
        ...
        struct npc_saurfang_gunshipAI : public ScriptedAI
        {
            ...
            void DamageTaken(Unit* /*attacker*/, uint32& damage)
            {
                if (m_team == ALLIANCE && me->GetHealthPct() < 2.0f )
                {
                    damage = 0;
                }
                
                if (m_team == HORDE && me->GetHealthPct() < 2.0f )
                {
                    damage = 0;
                    me->AI()->DoAction(ACTION_FAIL);
                }
            ...
            void UpdateAI(const uint32 diff)
            {
                ...
                if(_instance->GetBossState(DATA_GUNSHIP_EVENT) == IN_PROGRESS)
                {
                    if (!HealthAbovePct(75) && m_team == ALLIANCE)
                    {
                        me->SetHealth(me->GetMaxHealth() / 100 * 76); // find a better way to avoid the hardcore spell spam ....
                        DoCast(me, SPELL_TASTE_OF_BLOOD);
                    } 
                ...
                if (CombatBoxCheck)
                {
                    if (me->getVictim())
                        if (m_team == ALLIANCE)
                        {
                            if(isOutOfGunshipOrgrimmarCombatbox(false, me->GetPositionX(), me->GetPositionY()))
                                EnterEvadeMode();
                        }
    ...
    public:
        ...
        class spell_icc_remove_rocket_pack_SpellScript : public SpellScript
        {
            ...
            void HandleEffect(SpellEffIndex /*effIndex*/)
            {
                ...
                uint32 itemCount = hitPlr->GetItemCount(itemId, true); // Should be 1, but just in case.
                hitPlr->DestroyItemCount(itemId, itemCount, true, true);
            }
