...
void AuctionHouseBot::addNewAuctions(Player *AHBplayer, AHBConfig *config)
{
    if (!AHBSeller)
    {
        if (debug_Out) sLog->outError(LOG_FILTER_GENERAL,"AHSeller: Disabled");
        return;
    }
    ...
    {
        //if (debug_Out) sLog->outInfo(LOG_FILTER_GENERAL, "AHSeller: Auctions above minimum");
        return;
    }

    if (auctions >= maxItems)
    {
        //if (debug_Out) sLog->outInfo(LOG_FILTER_GENERAL, "AHSeller: Auctions at or above maximum");
        return;
    }
    ...
    if (debug_Out) sLog->outInfo(LOG_FILTER_GENERAL, "AHSeller: Current Auctineer GUID is %u", AuctioneerGUID);
...
*/
    ...
    if (debug_Out) sLog->outInfo(LOG_FILTER_GENERAL, "AHSeller: %u items", items);

    // only insert a few at a time, so as not to peg the processor
    for (uint32 cnt = 1; cnt <= items; cnt++)
    {
    if (debug_Out) sLog->outInfo(LOG_FILTER_GENERAL, "AHSeller: %u count", cnt);
        uint32 itemID = 0;
        uint32 itemColor = 99;
...
void AuctionHouseBot::addNewAuctionBuyerBotBid(Player *AHBplayer, AHBConfig *config, WorldSession *session)
{
    ...
    for (uint32 count = 1; count <= config->GetBidsPerInterval(); ++count)
    {
        ...
        {
            //if (debug_Out) sLog->outInfo(LOG_FILTER_GENERAL, "AHBuyer: I have no items to bid on.");
        ...
        if (debug_Out)
        {
            ...
            sLog->outInfo(LOG_FILTER_GENERAL, "AHBuyer: Expire Time: %u", auction->expire_time);
            sLog->outInfo(LOG_FILTER_GENERAL, "AHBuyer: Bid Rate: %f", bidrate);
            sLog->outInfo(LOG_FILTER_GENERAL, "AHBuyer: Bid Max: %f", bidMax);
            sLog->outInfo(LOG_FILTER_GENERAL, "AHBuyer: Bid Value: %f", bidvalue);
...
void AuctionHouseBot::Update()
{
    ...
    if (!sWorld->getBoolConfig(CONFIG_ALLOW_TWO_SIDE_INTERACTION_AUCTION))
    {
        ...
        {
            //if (debug_Out) sLog->outInfo(LOG_FILTER_GENERAL, "AHBuyer: %u seconds have passed since last bid", (_newrun - _lastrun_a));
            //if (debug_Out) sLog->outInfo(LOG_FILTER_GENERAL, "AHBuyer: Bidding on Alliance Auctions");
        ...
        {
            //if (debug_Out) sLog->outInfo(LOG_FILTER_GENERAL, "AHBuyer: %u seconds have passed since last bid", (_newrun - _lastrun_h));
            //if (debug_Out) sLog->outInfo(LOG_FILTER_GENERAL, "AHBuyer: Bidding on Horde Auctions");
    ...
    {
        //if (debug_Out) sLog->outInfo(LOG_FILTER_GENERAL, "AHBuyer: %u seconds have passed since last bid", (_newrun - _lastrun_n));
        //if (debug_Out) sLog->outInfo(LOG_FILTER_GENERAL, "AHBuyer: Bidding on Neutral Auctions");
...
void AuctionHouseBot::Initialize()
{
    ...
    if ((AHBplayerAccount != 0) || (AHBplayerGUID != 0))
    {
        ...
        if (!result)
       {
           sLog->outError(LOG_FILTER_GENERAL,"AuctionHouseBot: The account/GUID-information set for your AHBot is incorrect (account: %u guid: %u)", AHBplayerAccount, AHBplayerGUID);
           return;
        }
    }
    ...
    if (AHBSeller)
    {
        ...
        {
            if (debug_Out) sLog->outInfo(LOG_FILTER_GENERAL, "AuctionHouseBot: \"%s\" failed", npcQuery);
        }
        ...
        {
            if (debug_Out) sLog->outInfo(LOG_FILTER_GENERAL, "AuctionHouseBot: \"%s\" failed", lootQuery);
        }
        ...
        for (ItemTemplateContainer::const_iterator itr = its->begin(); itr != its->end(); ++itr)
        {
            ...
            {
                if (debug_Out_Filters) sLog->outInfo(LOG_FILTER_GENERAL, "AuctionHouseBot: Item %u disabled (PTR/Beta/Unused Item)", itr->second.ItemId);
                continue;
            }
            ...
            {
                if (debug_Out_Filters) sLog->outInfo(LOG_FILTER_GENERAL, "AuctionHouseBot: Item %u disabled (Permanent Enchant Item)", itr->second.ItemId);
                continue;
            }
            ...
            {
                if (debug_Out_Filters) sLog->outInfo(LOG_FILTER_GENERAL, "AuctionHouseBot: Item %u disabled (Conjured Consumable)", itr->second.ItemId);
                continue;
            }
            ...
            {
                if (debug_Out_Filters) sLog->outInfo(LOG_FILTER_GENERAL, "AuctionHouseBot: Item %u disabled (Gem)", itr->second.ItemId);
                continue;
            }
            ...
            {
                if (debug_Out_Filters) sLog->outInfo(LOG_FILTER_GENERAL, "AuctionHouseBot: Item %u disabled (Money)", itr->second.ItemId);
                continue;
            }
            ...
            {
                if (debug_Out_Filters) sLog->outInfo(LOG_FILTER_GENERAL, "AuctionHouseBot: Item %u disabled (MoneyLoot)", itr->second.ItemId);
                continue;
            }
            ...
            {
                if (debug_Out_Filters) sLog->outInfo(LOG_FILTER_GENERAL, "AuctionHouseBot: Item %u disabled (Lootable Item)", itr->second.ItemId);
                continue;
            }
            ...
            {
                if (debug_Out_Filters) sLog->outInfo(LOG_FILTER_GENERAL, "AuctionHouseBot: Item %u disabled (Quest Item)", itr->second.ItemId);
                continue;
            }
            ...
            {
                if (debug_Out_Filters) sLog->outInfo(LOG_FILTER_GENERAL, "AuctionHouseBot: Item %u disabled (Has a Duration)", itr->second.ItemId);
                continue;
            }
            ...
            {
                if (debug_Out_Filters) sLog->outInfo(LOG_FILTER_GENERAL, "AuctionHouseBot: Item %u disabled (BOP or BQI and Required Level is less than Item Level)", itr->second.ItemId);
                continue;
            }
            ...
            {
                if (debug_Out_Filters) sLog->outInfo(LOG_FILTER_GENERAL, "AuctionHouseBot: Item %u disabled (Warrior Item)", itr->second.ItemId);
                continue;
            }
            ...
            {
                if (debug_Out_Filters) sLog->outInfo(LOG_FILTER_GENERAL, "AuctionHouseBot: Item %u disabled (Paladin Item)", itr->second.ItemId);
                continue;
            }
            ...
            {
                if (debug_Out_Filters) sLog->outInfo(LOG_FILTER_GENERAL, "AuctionHouseBot: Item %u disabled (Hunter Item)", itr->second.ItemId);
                continue;
            }
            ...
            {
                if (debug_Out_Filters) sLog->outInfo(LOG_FILTER_GENERAL, "AuctionHouseBot: Item %u disabled (Rogue Item)", itr->second.ItemId);
                continue;
            }
            ...
            {
                if (debug_Out_Filters) sLog->outInfo(LOG_FILTER_GENERAL, "AuctionHouseBot: Item %u disabled (Priest Item)", itr->second.ItemId);
                continue;
            }
            ...
            {
                if (debug_Out_Filters) sLog->outInfo(LOG_FILTER_GENERAL, "AuctionHouseBot: Item %u disabled (DK Item)", itr->second.ItemId);
                continue;
            }
            ...
            {
                if (debug_Out_Filters) sLog->outInfo(LOG_FILTER_GENERAL, "AuctionHouseBot: Item %u disabled (Shaman Item)", itr->second.ItemId);
                continue;
            }
            ...
            {
                if (debug_Out_Filters) sLog->outInfo(LOG_FILTER_GENERAL, "AuctionHouseBot: Item %u disabled (Mage Item)", itr->second.ItemId);
                continue;
            }
            ...
            {
                if (debug_Out_Filters) sLog->outInfo(LOG_FILTER_GENERAL, "AuctionHouseBot: Item %u disabled (Warlock Item)", itr->second.ItemId);
                continue;
            }
            ...
            {
                if (debug_Out_Filters) sLog->outInfo(LOG_FILTER_GENERAL, "AuctionHouseBot: Item %u disabled (Unused Item)", itr->second.ItemId);
                continue;
            }
            ...
            {
                if (debug_Out_Filters) sLog->outInfo(LOG_FILTER_GENERAL, "AuctionHouseBot: Item %u disabled (Druid Item)", itr->second.ItemId);
                continue;
            }
            ...
            {
                if (debug_Out_Filters) sLog->outInfo(LOG_FILTER_GENERAL, "AuctionHouseBot: Item %u disabled (Item Level = %u)", itr->second.ItemId, itr->second.ItemLevel);
                continue;
            }
           ...
           // Disable Trade Goods below level X
            ...
            {
                if (debug_Out_Filters) sLog->outInfo(LOG_FILTER_GENERAL, "AuctionHouseBot: Trade Good %u disabled (Trade Good Level = %u)", itr->second.ItemId, itr->second.ItemLevel);
                continue;
            }
           ...
           // Disable Trade Goods above level X
            ...
            {
                if (debug_Out_Filters) sLog->outInfo(LOG_FILTER_GENERAL, "AuctionHouseBot: Item %u disabled (Trade Good Level = %u)", itr->second.ItemId, itr->second.ItemLevel);
                continue;
            }
            ...
            {
                if (debug_Out_Filters) sLog->outInfo(LOG_FILTER_GENERAL, "AuctionHouseBot: Item %u disabled (RequiredLevel = %u)", itr->second.ItemId, itr->second.RequiredLevel);
                continue;
            }
            ...
            {
                if (debug_Out_Filters) sLog->outInfo(LOG_FILTER_GENERAL, "AuctionHouseBot: Trade Good %u disabled (RequiredLevel = %u)", itr->second.ItemId, itr->second.RequiredLevel);
                continue;
            }
            ...
            // {
            //    if (debug_Out_Filters) sLog->outInfo(LOG_FILTER_GENERAL, "AuctionHouseBot: Item %u disabled (RequiredSkillRank = %u)", itr->second.ItemId, itr->second.RequiredSkillRank);
            //    continue;
            // }
        ...
        {
            sLog->outError(LOG_FILTER_GENERAL,"AuctionHouseBot: No items");
            AHBSeller = 0;
        }

        sLog->outInfo(LOG_FILTER_GENERAL, "AuctionHouseBot:");
        sLog->outInfo(LOG_FILTER_GENERAL, "loaded %u grey trade goods", uint32(greyTradeGoodsBin.size()));
        sLog->outInfo(LOG_FILTER_GENERAL, "loaded %u white trade goods", uint32(whiteTradeGoodsBin.size()));
        sLog->outInfo(LOG_FILTER_GENERAL, "loaded %u green trade goods", uint32(greenTradeGoodsBin.size()));
        sLog->outInfo(LOG_FILTER_GENERAL, "loaded %u blue trade goods", uint32(blueTradeGoodsBin.size()));
        sLog->outInfo(LOG_FILTER_GENERAL, "loaded %u purple trade goods", uint32(purpleTradeGoodsBin.size()));
        sLog->outInfo(LOG_FILTER_GENERAL, "loaded %u orange trade goods", uint32(orangeTradeGoodsBin.size()));
        sLog->outInfo(LOG_FILTER_GENERAL, "loaded %u yellow trade goods", uint32(yellowTradeGoodsBin.size()));
        sLog->outInfo(LOG_FILTER_GENERAL, "loaded %u grey items", uint32(greyItemsBin.size()));
        sLog->outInfo(LOG_FILTER_GENERAL, "loaded %u white items", uint32(whiteItemsBin.size()));
        sLog->outInfo(LOG_FILTER_GENERAL, "loaded %u green items", uint32(greenItemsBin.size()));
        sLog->outInfo(LOG_FILTER_GENERAL, "loaded %u blue items", uint32(blueItemsBin.size()));
        sLog->outInfo(LOG_FILTER_GENERAL, "loaded %u purple items", uint32(purpleItemsBin.size()));
        sLog->outInfo(LOG_FILTER_GENERAL, "loaded %u orange items", uint32(orangeItemsBin.size()));
        sLog->outInfo(LOG_FILTER_GENERAL, "loaded %u yellow items", uint32(yellowItemsBin.size()));

    }
    sLog->outInfo(LOG_FILTER_GENERAL, "AuctionHouseBot and AuctionHouseBuyer have been loaded.");
}
...
{
    if (debug_Out) sLog->outInfo(LOG_FILTER_GENERAL, ((std::string)("Start Settings for %s Auctionhouses:", CharacterDatabase.PQuery("SELECT name FROM auctionhousebot WHERE auctionhouse = %u",config->GetAHID())->Fetch()->GetString())).c_str());
    if (AHBSeller)
    {
        ...
        if (debug_Out)
        {
            sLog->outInfo(LOG_FILTER_GENERAL, "minItems                = %u", config->GetMinItems());
            sLog->outInfo(LOG_FILTER_GENERAL, "maxItems                = %u", config->GetMaxItems());
            sLog->outInfo(LOG_FILTER_GENERAL, "percentGreyTradeGoods   = %u", config->GetPercentages(AHB_GREY_TG));
            sLog->outInfo(LOG_FILTER_GENERAL, "percentWhiteTradeGoods  = %u", config->GetPercentages(AHB_WHITE_TG));
            sLog->outInfo(LOG_FILTER_GENERAL, "percentGreenTradeGoods  = %u", config->GetPercentages(AHB_GREEN_TG));
            sLog->outInfo(LOG_FILTER_GENERAL, "percentBlueTradeGoods   = %u", config->GetPercentages(AHB_BLUE_TG));
            sLog->outInfo(LOG_FILTER_GENERAL, "percentPurpleTradeGoods = %u", config->GetPercentages(AHB_PURPLE_TG));
            sLog->outInfo(LOG_FILTER_GENERAL, "percentOrangeTradeGoods = %u", config->GetPercentages(AHB_ORANGE_TG));
            sLog->outInfo(LOG_FILTER_GENERAL, "percentYellowTradeGoods = %u", config->GetPercentages(AHB_YELLOW_TG));
            sLog->outInfo(LOG_FILTER_GENERAL, "percentGreyItems        = %u", config->GetPercentages(AHB_GREY_I));
            sLog->outInfo(LOG_FILTER_GENERAL, "percentWhiteItems       = %u", config->GetPercentages(AHB_WHITE_I));
            sLog->outInfo(LOG_FILTER_GENERAL, "percentGreenItems       = %u", config->GetPercentages(AHB_GREEN_I));
            sLog->outInfo(LOG_FILTER_GENERAL, "percentBlueItems        = %u", config->GetPercentages(AHB_BLUE_I));
            sLog->outInfo(LOG_FILTER_GENERAL, "percentPurpleItems      = %u", config->GetPercentages(AHB_PURPLE_I));
            sLog->outInfo(LOG_FILTER_GENERAL, "percentOrangeItems      = %u", config->GetPercentages(AHB_ORANGE_I));
            sLog->outInfo(LOG_FILTER_GENERAL, "percentYellowItems      = %u", config->GetPercentages(AHB_YELLOW_I));
            sLog->outInfo(LOG_FILTER_GENERAL, "minPriceGrey            = %u", config->GetMinPrice(AHB_GREY));
            sLog->outInfo(LOG_FILTER_GENERAL, "maxPriceGrey            = %u", config->GetMaxPrice(AHB_GREY));
            sLog->outInfo(LOG_FILTER_GENERAL, "minPriceWhite           = %u", config->GetMinPrice(AHB_WHITE));
            sLog->outInfo(LOG_FILTER_GENERAL, "maxPriceWhite           = %u", config->GetMaxPrice(AHB_WHITE));
            sLog->outInfo(LOG_FILTER_GENERAL, "minPriceGreen           = %u", config->GetMinPrice(AHB_GREEN));
            sLog->outInfo(LOG_FILTER_GENERAL, "maxPriceGreen           = %u", config->GetMaxPrice(AHB_GREEN));
            sLog->outInfo(LOG_FILTER_GENERAL, "minPriceBlue            = %u", config->GetMinPrice(AHB_BLUE));
            sLog->outInfo(LOG_FILTER_GENERAL, "maxPriceBlue            = %u", config->GetMaxPrice(AHB_BLUE));
            sLog->outInfo(LOG_FILTER_GENERAL, "minPricePurple          = %u", config->GetMinPrice(AHB_PURPLE));
            sLog->outInfo(LOG_FILTER_GENERAL, "maxPricePurple          = %u", config->GetMaxPrice(AHB_PURPLE));
            sLog->outInfo(LOG_FILTER_GENERAL, "minPriceOrange          = %u", config->GetMinPrice(AHB_ORANGE));
            sLog->outInfo(LOG_FILTER_GENERAL, "maxPriceOrange          = %u", config->GetMaxPrice(AHB_ORANGE));
            sLog->outInfo(LOG_FILTER_GENERAL, "minPriceYellow          = %u", config->GetMinPrice(AHB_YELLOW));
            sLog->outInfo(LOG_FILTER_GENERAL, "maxPriceYellow          = %u", config->GetMaxPrice(AHB_YELLOW));
            sLog->outInfo(LOG_FILTER_GENERAL, "minBidPriceGrey         = %u", config->GetMinBidPrice(AHB_GREY));
            sLog->outInfo(LOG_FILTER_GENERAL, "maxBidPriceGrey         = %u", config->GetMaxBidPrice(AHB_GREY));
            sLog->outInfo(LOG_FILTER_GENERAL, "minBidPriceWhite        = %u", config->GetMinBidPrice(AHB_WHITE));
            sLog->outInfo(LOG_FILTER_GENERAL, "maxBidPriceWhite        = %u", config->GetMaxBidPrice(AHB_WHITE));
            sLog->outInfo(LOG_FILTER_GENERAL, "minBidPriceGreen        = %u", config->GetMinBidPrice(AHB_GREEN));
            sLog->outInfo(LOG_FILTER_GENERAL, "maxBidPriceGreen        = %u", config->GetMaxBidPrice(AHB_GREEN));
            sLog->outInfo(LOG_FILTER_GENERAL, "minBidPriceBlue         = %u", config->GetMinBidPrice(AHB_BLUE));
            sLog->outInfo(LOG_FILTER_GENERAL, "maxBidPriceBlue         = %u", config->GetMinBidPrice(AHB_BLUE));
            sLog->outInfo(LOG_FILTER_GENERAL, "minBidPricePurple       = %u", config->GetMinBidPrice(AHB_PURPLE));
            sLog->outInfo(LOG_FILTER_GENERAL, "maxBidPricePurple       = %u", config->GetMaxBidPrice(AHB_PURPLE));
            sLog->outInfo(LOG_FILTER_GENERAL, "minBidPriceOrange       = %u", config->GetMinBidPrice(AHB_ORANGE));
            sLog->outInfo(LOG_FILTER_GENERAL, "maxBidPriceOrange       = %u", config->GetMaxBidPrice(AHB_ORANGE));
            sLog->outInfo(LOG_FILTER_GENERAL, "minBidPriceYellow       = %u", config->GetMinBidPrice(AHB_YELLOW));
            sLog->outInfo(LOG_FILTER_GENERAL, "maxBidPriceYellow       = %u", config->GetMaxBidPrice(AHB_YELLOW));
            sLog->outInfo(LOG_FILTER_GENERAL, "maxStackGrey            = %u", config->GetMaxStack(AHB_GREY));
            sLog->outInfo(LOG_FILTER_GENERAL, "maxStackWhite           = %u", config->GetMaxStack(AHB_WHITE));
            sLog->outInfo(LOG_FILTER_GENERAL, "maxStackGreen           = %u", config->GetMaxStack(AHB_GREEN));
            sLog->outInfo(LOG_FILTER_GENERAL, "maxStackBlue            = %u", config->GetMaxStack(AHB_BLUE));
            sLog->outInfo(LOG_FILTER_GENERAL, "maxStackPurple          = %u", config->GetMaxStack(AHB_PURPLE));
            sLog->outInfo(LOG_FILTER_GENERAL, "maxStackOrange          = %u", config->GetMaxStack(AHB_ORANGE));
            sLog->outInfo(LOG_FILTER_GENERAL, "maxStackYellow          = %u", config->GetMaxStack(AHB_YELLOW));
        }
        //AuctionHouseEntry const* ahEntry =  sAuctionMgr->GetAuctionHouseEntry(config->GetAHFID());
        AuctionHouseObject* auctionHouse =  sAuctionMgr->GetAuctionsMap(config->GetAHFID());
        ...
        if (debug_Out)
        {
            sLog->outInfo(LOG_FILTER_GENERAL, ((std::string)("Current Settings for %s Auctionhouses:", CharacterDatabase.PQuery("SELECT name FROM auctionhousebot WHERE auctionhouse = %u",config->GetAHID())->Fetch()->GetString())).c_str());
            sLog->outInfo(LOG_FILTER_GENERAL, "Grey Trade Goods\t%u\tGrey Items\t%u", config->GetItemCounts(AHB_GREY_TG), config->GetItemCounts(AHB_GREY_I));
            sLog->outInfo(LOG_FILTER_GENERAL, "White Trade Goods\t%u\tWhite Items\t%u", config->GetItemCounts(AHB_WHITE_TG), config->GetItemCounts(AHB_WHITE_I));
            sLog->outInfo(LOG_FILTER_GENERAL, "Green Trade Goods\t%u\tGreen Items\t%u", config->GetItemCounts(AHB_GREEN_TG), config->GetItemCounts(AHB_GREEN_I));
            sLog->outInfo(LOG_FILTER_GENERAL, "Blue Trade Goods\t%u\tBlue Items\t%u", config->GetItemCounts(AHB_BLUE_TG), config->GetItemCounts(AHB_BLUE_I));
            sLog->outInfo(LOG_FILTER_GENERAL, "Purple Trade Goods\t%u\tPurple Items\t%u", config->GetItemCounts(AHB_PURPLE_TG), config->GetItemCounts(AHB_PURPLE_I));
            sLog->outInfo(LOG_FILTER_GENERAL, "Orange Trade Goods\t%u\tOrange Items\t%u", config->GetItemCounts(AHB_ORANGE_TG), config->GetItemCounts(AHB_ORANGE_I));
            sLog->outInfo(LOG_FILTER_GENERAL, "Yellow Trade Goods\t%u\tYellow Items\t%u", config->GetItemCounts(AHB_YELLOW_TG), config->GetItemCounts(AHB_YELLOW_I));
        }
    }
    if (AHBBuyer)
    {
        ...
        if (debug_Out)
        {
            sLog->outInfo(LOG_FILTER_GENERAL, "buyerPriceGrey          = %u", config->GetBuyerPrice(AHB_GREY));
            sLog->outInfo(LOG_FILTER_GENERAL, "buyerPriceWhite         = %u", config->GetBuyerPrice(AHB_WHITE));
            sLog->outInfo(LOG_FILTER_GENERAL, "buyerPriceGreen         = %u", config->GetBuyerPrice(AHB_GREEN));
            sLog->outInfo(LOG_FILTER_GENERAL, "buyerPriceBlue          = %u", config->GetBuyerPrice(AHB_BLUE));
            sLog->outInfo(LOG_FILTER_GENERAL, "buyerPricePurple        = %u", config->GetBuyerPrice(AHB_PURPLE));
            sLog->outInfo(LOG_FILTER_GENERAL, "buyerPriceOrange        = %u", config->GetBuyerPrice(AHB_ORANGE));
            sLog->outInfo(LOG_FILTER_GENERAL, "buyerPriceYellow        = %u", config->GetBuyerPrice(AHB_YELLOW));
            sLog->outInfo(LOG_FILTER_GENERAL, "buyerBiddingInterval    = %u", config->GetBiddingInterval());
            sLog->outInfo(LOG_FILTER_GENERAL, "buyerBidsPerInterval    = %u", config->GetBidsPerInterval());
        }
    }
    if (debug_Out) sLog->outInfo(LOG_FILTER_GENERAL, ((std::string)("End Settings for %s Auctionhouses:", CharacterDatabase.PQuery("SELECT name FROM auctionhousebot WHERE auctionhouse = %u",config->GetAHID())->Fetch()->GetString())).c_str());
}
