...
bool GuildHouseObject::ChangeGuildHouse(uint32 guild_id, uint32 newid)
{
    ...
    {        
        ...
        if (itr == GH_map.end())
            return true;
        QueryResult result = CharacterDatabase.PQuery("UPDATE `guildhouses` SET `guildId` = 0 WHERE `guildId` = %u", guild_id);
    ...
    else // Compra
    {
        ...
        result = CharacterDatabase.PQuery("SELECT `x`, `y`, `z`, `map`, `minguildsize`, `price` FROM `guildhouses` WHERE `id` = %u", newid);
        if (!result)
        ...
        result = CharacterDatabase.PQuery("UPDATE `guildhouses` SET `guildId` = %u WHERE `id` = %u", guild_id, newid);
...
{
    GH_map.clear();
    QueryResult result = CharacterDatabase.Query("SELECT `id`,`guildId`,`x`,`y`,`z`,`map`,`minguildsize`,`price` FROM guildhouses ORDER BY guildId ASC");
...
void GuildHouseObject::LoadGuildHouseAdd()
{
    ...
    QueryResult result = CharacterDatabase.Query("SELECT `guid`,`type`,`id`,`add_type` FROM guildhouses_add ORDER BY Id ASC");
